...
public class PaymentFailedProducer {
    ...
    public void publishPaymentFailedEvent(String topicName, String key, PaymentFailedEvent event) {
        ...
        kafkaTemplate.send(producerRecord)
                .whenComplete((result, exception) -> {
                    if (exception != null) {
                        log.error("Failed to send payment failed event to topic: {}", topicName, exception);
                    } else {
                        log.info("Payment failed event sent successfully to topic: {}", topicName);
                    }
                });
    }
}
